...
int main(int argc , char** argv) {
  ...
  string dir ;
  int nbpop, N ;
  double K,duration,dt,Tst,Tw,**J,**Tsyn,g,*Iext,*IextBL,**Crec,Cff ;
  
  Set_Parameters(argc,argv,dir,nbpop,N,K,duration,dt,Tst,Tw,g,Iext,IextBL) ;

  Crec = new double* [nbpop] ;
  for(int i=0;i<nbpop;i++) {
    Crec[i] = new double[nbpop] ;
    for(int j=0;j<nbpop;j++) 
      Crec[i][j] = (double) atof(argv[2*nbpop+10+j+i*nbpop]) ;
  }
  
  if(nbpop==2)
    Cff = (double) atof(argv[18]) ; // variance of the wrapped external input
  if(nbpop==3)
    Cff = (double) atof(argv[25]) ; // variance of the wrapped external input
  ...
  CreateDir_SpaceCrecCab(nbpop,path,N,Crec) ;
  ...
  double Vr = 0. ; // Membrane Resting Potential
  double Vth = 1. ; // Voltage Threshold
  double Vpeak = 20. ; // Spikes Peak
  ...
  for (double t=0.; t<duration; t+=dt) {  
	...
	if (Volt[i][k]>=Vth) {
	  ...
	  if(t>=Tst and k<10) fVoltage << " " << Volt[i][k] ;
      } // endfor neurons
    } // endfor populations
    
    if(t>=Tst) fVoltage << endl ;
    
    // Updating Total Synaptic Input to each neurons in each population 
    for(i=0;i<nbpop;i++) 
      for (k=0;k<Nk[i];k++) 
	if(i==ndI && IF_PULSE) 
	  Isyntot[i][k] = IextBL[i] + Jext[i][k]*cos(M_PI*PULSE*t)*cos(M_PI*PULSE*t) ;
	else
