...
int main(int argc , char** argv) {
  ...
  int nbpop, ndI ; 
  unsigned long N ;  
  double K, **J, **Tsyn, g, *Iext, *IextBL, *Crec, Cff = 4.0*L ; 
  double *RatesMF ;
 ...
 ///////////////////////////////////////////////////////////////////    
  ...
  normal_distribution<double> gaussianV( (Vth-Vr)/2.0, (Vth-Vr)/16.0 ) ;
  normal_distribution<double> gaussianI( 0, 1.0 ) ; 
  ...
  for(i=0;i<nbpop;i++) 
    for(k=0;k<nbN[i];k++) { 
      ...
      Volt[i][k] = RatesMF[i] + gaussianV(gen) ; 
      for(j=0;j<nbpop;j++) 
	Isyn[i][j][k] = (K * J[i][j] * Tsyn[i][j] * RatesMF[j] + gaussianI(gen) ) / (double) nbN[i]; 
    }
  ...
  for (double t=0.; t<=duration+Tst+dt; t+=dt) { 
    ...
    for(i=0;i<nbpop;i++) 
      for(j=0;j<nbpop;j++) 
	    ...
	    Isyn[i][j][k] = Cstsyn[i][j]*Isyn[i][j][k] ; 
    	    // Isyn[i][j][k] = exp(-(tspk[j][k]-t)/Tsyn[i][j]) * Isyn[i][j][k] ; 
	...
	if (Volt[i][k]>=Vth) {
	  ...
	  Volt[i][k] = (Volt[i][k]-Vth)*(1.+dt/Tm[i]*(Vold[i][k]-Vr)/(Volt[i][k]-Vold[i][k])) + Vr ; 
	  ...
	  for(j=0;j<nbpop;j++) // i Pres to j Post => Jji 
		...
		if(IdPost[l]>=Cpt[j] && IdPost[l]<Cpt[j+1]) 
		  Isyn[j][i][IdPost[l]-Cpt[j]] += J[j][i] * exp(-(tspk[i][k]-t)/Tsyn[j][i]) ; 
