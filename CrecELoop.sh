...
if [ -z "$1" ]; then
    ...
    if [ "$nbpop" = 2 ]; then 
	read E I Tme Tmi CrecE CrecI Cff dCrecE CrecEmax <<< "${10} ${11} ${12} ${13} ${14} ${15} ${16} ${17} ${18}"
    fi
...
echo CrecEmin$CrecEmin dCrecE$dCrecE CrecEmax$CrecEmax
for CrecE in $(seq $CrecEmin $dCrecE $CrecEmax); do
